...
This repo packages [Lightning Terminal](https://github.com/lightninglabs/lightning-terminal) for StartOS.
...
Lightning Terminal is pulled in as the upstream `lightninglabs/lightning-terminal` Docker image; there is no Dockerfile in this repo. To track a new upstream release:

1. Bump `dockerTag` in `startos/manifest/index.ts` to `lightninglabs/lightning-terminal:v<new version>`.
