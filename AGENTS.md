...
- **Reaching LND goes through the LXC bridge**, not `.startos` DNS. `main.ts` resolves LND's gRPC `host:port` with `sdk.host.get({ hostId: gRPCHostId, packageId: 'lnd' }, …)` (ids imported from `lnd-startos/startos/interfaces`) and writes it into `remote.lnd.rpcserver` in `.lit/lit.conf` before the daemon starts; litd binds its own UI on `0.0.0.0:8443`.
