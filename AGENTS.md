...
- **`main`'s ready fn is also litd's watchdog — an upstream bump must re-verify its assumptions.** It kills a parked litd (`kill $(pidof litd)` — busybox `pkill` matches argv, not comm, and can never match `/bin/litd`) so the supervisor restarts it; the parked state is matched by message prefix because litd's status API offers no structural discriminator. The prefix, the two retry-loop messages that must never match, and the `terminal.go` sites to re-check live at `PARKED_PREFIX` in `startos/healthCheck.ts`. A bump must also confirm `sh`, `kill`, and `pidof` are still in the image.
